...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 778B4133080
	for <lists+dmaengine@lfdr.de>; Tue,  7 Jan 2020 21:18:28 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1728384AbgAGUS1 (ORCPT <rfc822;lists+dmaengine@lfdr.de>);
        Tue, 7 Jan 2020 15:18:27 -0500
Received: from mga06.intel.com ([134.134.136.31]:58879 "EHLO mga06.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1728379AbgAGUS1 (ORCPT <rfc822;dmaengine@vger.kernel.org>);
        Tue, 7 Jan 2020 15:18:27 -0500
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from orsmga006.jf.intel.com ([10.7.209.51])
  by orsmga104.jf.intel.com with ESMTP/TLS/DHE-RSA-AES256-GCM-SHA384; 07 Jan 2020 12:18:25 -0800
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.69,407,1571727600"; 
   d="scan'208";a="222714896"
Received: from djiang5-desk3.ch.intel.com ([143.182.136.137])
  by orsmga006.jf.intel.com with ESMTP; 07 Jan 2020 12:18:24 -0800
...
Message-ID: <c7ba628c-4ffe-fec9-6832-8139bd7865db@intel.com>
Date:   Tue, 7 Jan 2020 13:18:24 -0700
User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:68.0) Gecko/20100101
 Thunderbird/68.2.2
MIME-Version: 1.0
In-Reply-To: <94878eba-42c2-7b33-d315-a90b225606aa@intel.com>
...
>>>> Create a char device region that will allow acquisition of user 
>>>> portals in
>>>> order to allow applications to submit DMA operations. A char device 
>>>> will be
...
>>>> workqueue 0 of DSA device 0 is marked for char device, then a device 
>>>> node
...
traditional DMA engines. For idxd, the char device allows the user app 
to mmap() a 4k region called a portal. With that portal address the user 
can use either MOVDIR64B or ENQCMD CPU instruction to directly submit a 
DMA descriptor to the device without touching the kernel. I think this 
part is device specific and probably can only be provided by the driver 
for now.

I think you on the other hand are looking for a kernel assisted DMA 
operation submission from userspace through the dmaengine framework 
correct? We would expose a char device for a channel that allows a user 
app to submit DMA operations via ioctls? That is something the idxd 
driver needs as well when the operation hits a point where the user 
portal is saturated and we need kernel assistance to move forward. But I 
would like to mark this as follow on work for me to add to dmaengine as 
a common framework.
...
>>>> +}
>>>> +
>>>> +static inline struct idxd_wq *idxd_cdev_wq(struct idxd_cdev 
>>>> *idxd_cdev)
>>>> +{
...
>>>> +}
>>>> +
>>>> +static int idxd_cdev_mmap(struct file *filp, struct vm_area_struct 
>>>> *vma)
>>>> +{
...
>>>> +    minor = ida_simple_get(&cdev_ctx->minor_ida, 0, MINORMASK, 
>>>> GFP_KERNEL);
>>>> +    if (minor < 0) {
>>>> +        rc = minor;
>>>> +        goto ida_err;
...
>>>> +    /*
>>>> +     * The device_type->release() will be called on the device and 
>>>> free
...
>>>>    #define confdev_to_idxd(dev) container_of(dev, struct 
>>>> idxd_device, conf_dev)
>>>>    #define confdev_to_wq(dev) container_of(dev, struct idxd_wq, 
>>>> conf_dev)
...
>>>> +};
>>>> +
>>>> +static inline int idxd_get_wq_portal_offset(enum idxd_portal_prot 
>>>> prot)
>>>> +{
>>>> +    return prot * 0x1000;
>>>> +}
...
>>>> @@ -267,4 +297,12 @@ void idxd_unregister_dma_device(struct 
>>>> idxd_device *idxd);
>>>>    int idxd_register_dma_channel(struct idxd_wq *wq);
>>>>    void idxd_unregister_dma_channel(struct idxd_wq *wq);
>>>>    void idxd_parse_completion_status(u8 status, enum 
>>>> dmaengine_tx_result *res);
>>>> +
>>>> +/* cdev */
>>>> +int idxd_cdev_register(void);
>>>> +void idxd_cdev_remove(void);
...
>>>> @@ -188,6 +188,7 @@ static int idxd_setup_internals(struct 
>>>> idxd_device *idxd)
...
>>>>            iowrite64(IDXD_SWERR_ACK, idxd->reg_base + 
>>>> IDXD_SWERR_OFFSET);
>>>> +
...
>>>> @@ -71,17 +71,19 @@ static int idxd_submit_desc(struct idxd_wq *wq, 
...
>>>>         * triggering a DMA read. The wmb() is necessary even on UP 
>>>> because
...
>>>> @@ -63,6 +64,11 @@ static inline bool is_idxd_wq_dmaengine(struct 
>>>> idxd_wq *wq)
>>>>        return false;
>>>>    }
...
>>>>    {
>>>> @@ -109,6 +115,9 @@ static int idxd_config_bus_probe(struct device 
>>>> *dev)
>>>>                return -EBUSY;
>>>>            }
...
>>>> @@ -216,6 +225,13 @@ static int idxd_config_bus_probe(struct device 
>>>> *dev)
...
>>>> @@ -295,10 +313,12 @@ static int idxd_config_bus_remove(struct 
>>>> device *dev)
>>>>            spin_lock_irqsave(&idxd->dev_lock, flags);
...
>>>>        }
>>>>        return 0;
>>>> @@ -309,7 +329,7 @@ static void idxd_config_bus_shutdown(struct 
>>>> device *dev)
>>>>        dev_dbg(dev, "%s called\n", __func__);
>>>>    }
...
>>>>        NULL,
>>>>    };
>>>> @@ -1242,6 +1279,16 @@ static ssize_t token_limit_store(struct 
>>>> device *dev,
>>>>    }
...
>>>> @@ -1257,6 +1304,7 @@ static struct attribute 

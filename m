...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 845922198A8
	for <lists+dmaengine@lfdr.de>; Thu,  9 Jul 2020 08:29:51 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726129AbgGIG3l (ORCPT <rfc822;lists+dmaengine@lfdr.de>);
        Thu, 9 Jul 2020 02:29:41 -0400
Received: from mga17.intel.com ([192.55.52.151]:36808 "EHLO mga17.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726006AbgGIG3k (ORCPT <rfc822;dmaengine@vger.kernel.org>);
        Thu, 9 Jul 2020 02:29:40 -0400
IronPort-SDR: eoTUoUElP4cKfBRvp8vmtYaUQmNdguKJ1CU890A5Z2JOAiMGCMCBmiF2uh9r3VAvpQiJEGdd4j
 ++rABwtCGzMA==
X-IronPort-AV: E=McAfee;i="6000,8403,9676"; a="128013007"
X-IronPort-AV: E=Sophos;i="5.75,330,1589266800"; 
   d="scan'208";a="128013007"
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from fmsmga007.fm.intel.com ([10.253.24.52])
  by fmsmga107.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 08 Jul 2020 23:03:18 -0700
IronPort-SDR: KCMBdNZx4gh9D0mv0IJ0ub0pZFBSpds3SdUubqY8w1nScF91vJBiL8Hp6DKrCeITFM9t+DVFdU
 WiKyLX335jqA==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.75,330,1589266800"; 
   d="scan'208";a="267321076"
Received: from sgsxdev004.isng.intel.com (HELO localhost) ([10.226.88.13])
  by fmsmga007.fm.intel.com with ESMTP; 08 Jul 2020 23:03:15 -0700
...
Subject: [PATCH v4 2/2] Add Intel LGM soc DMA support.
Date:   Thu,  9 Jul 2020 14:01:06 +0800
Message-Id: <6be9b9cfbf6708fe371f280cb94cbdc9c04bdccb.1594273437.git.mallikarjunax.reddy@linux.intel.com>
...
new file mode 100644
index 000000000000..91c0a28fe8fb
--- /dev/null
...
+	if (!fwnode_property_read_u32(fw_chan, "intel,pkt-sz", &val)) {
...
+		c->boff_len = 0;
+
+	if (fwnode_property_read_u32(fw_chan, "non-arb-cnt", &c->nonarb_cnt))
+		c->nonarb_cnt = 0;
+
+	if (fwnode_property_read_u32(fw_chan, "intel,arb-cnt", &c->arb_cnt))
+		c->arb_cnt = 0;
+
...
+	c->pden = fwnode_property_read_bool(fw_chan, "intel,pkt-drop");
+	if (c->pden) {
+		p = c->port;
...
+	p->chan_sz = v[1];
+
+	if (fwnode_property_read_u32(fw_port, "intel,burst", &p->txbl))
...
+	p->rxbl = p->txbl;
+
+	if (fwnode_property_read_u32(fw_port, "intel,endian", &p->txendi))
+		p->txendi = DMA_DFT_ENDIAN;
+
+	/* TX and RX has the same endianness */
+	p->rxendi = p->txendi;
+
+	if (fwnode_property_read_u32(fw_port, "intel,txwgt", &p->txwgt))

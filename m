...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 1D42A2A2D25
	for <lists+dmaengine@lfdr.de>; Mon,  2 Nov 2020 15:42:13 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726020AbgKBOls (ORCPT <rfc822;lists+dmaengine@lfdr.de>);
        Mon, 2 Nov 2020 09:41:48 -0500
Received: from mga04.intel.com ([192.55.52.120]:39857 "EHLO mga04.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1725788AbgKBOls (ORCPT <rfc822;dmaengine@vger.kernel.org>);
        Mon, 2 Nov 2020 09:41:48 -0500
IronPort-SDR: p/ibTrFBQGGQir4R6qNtB9H+WCPliKXojdLE3pj26TL3dY0HEESKQBcjjq+UcpZxMY2nzKV3Zu
 Ciyyv88Q/cAg==
X-IronPort-AV: E=McAfee;i="6000,8403,9792"; a="166301809"
X-IronPort-AV: E=Sophos;i="5.77,445,1596524400"; 
   d="scan'208";a="166301809"
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from fmsmga002.fm.intel.com ([10.253.24.26])
  by fmsmga104.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 02 Nov 2020 06:41:46 -0800
IronPort-SDR: N3n7a8hPoeTJB1OWE1V8KlLO80I3d6p47U16eVg3Rd1Pcyq9Whoav5boxlsGQlqasY+EGkUPw7
 bI3iNZlG2GEA==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.77,445,1596524400"; 
   d="scan'208";a="357346325"
Received: from linux.intel.com ([10.54.29.200])
  by fmsmga002.fm.intel.com with ESMTP; 02 Nov 2020 06:41:46 -0800
Received: from [10.249.65.176] (mreddy3x-MOBL.gar.corp.intel.com [10.249.65.176])
        by linux.intel.com (Postfix) with ESMTP id 7AF6E58082E;
        Mon,  2 Nov 2020 06:41:42 -0800 (PST)
...
References: <cover.1600827061.git.mallikarjunax.reddy@linux.intel.com>
 ...
 <DM6PR19MB359406F3ADFEA16AF9DAC565BB160@DM6PR19MB3594.namprd19.prod.outlook.com>
From:   "Reddy, MallikarjunaX" <mallikarjunax.reddy@linux.intel.com>
Message-ID: <10106652-7223-1c15-4f96-6cd68b71b890@linux.intel.com>
Date:   Mon, 2 Nov 2020 22:41:41 +0800
User-Agent: Mozilla/5.0 (Windows NT 10.0; WOW64; rv:68.0) Gecko/20100101
 Thunderbird/68.12.1
MIME-Version: 1.0
In-Reply-To: <DM6PR19MB359406F3ADFEA16AF9DAC565BB160@DM6PR19MB3594.namprd19.prod.outlook.com>
...
>> ming.wu@intel.com>; mallikarjunax.reddy@linux.intel.com;
...
>> +config INTEL_LDMA
>> +	bool "Lightning Mountain centralized low speed DMA and high speed
>> DMA controllers"
>> +	select DMA_ENGINE
>> +	select DMA_VIRTUAL_CHANNELS
>> +	help
>> +	  Enable support for intel Lightning Mountain SOC DMA
>> controllers.
>> +	  These controllers provide DMA capabilities for a variety of on-
>> chip
...
>> new file mode 100644
>> index 000000000000..bd9bd450a50a
>> --- /dev/null
>> +++ b/drivers/dma/lgm/lgm-dma.c
>> @@ -0,0 +1,1765 @@
>> +// SPDX-License-Identifier: GPL-2.0
>> +/*
>> + * Lightning Mountain centralized low speed and high speed DMA
>> controller driver
>> + *
...
> Why "ldma"? Same applies to most functions with the prefix "ldma_".
i will use file name as DRIVER_NAME.

#define DRIVER_NAME "lgm-dma"

is it ok or do you have any suggestions.
>
>> +
...
>> +#define DMA_CI_ALL							\
>> +	(DMA_CI_EOP | DMA_CI_DUR | DMA_CI_DESCPT | DMA_CI_CHOFF |
>> DMA_CI_RDERR)
>> +
...
>> + *   If bit 30 is disabled, HDR_LEN must be configured according to
>> channel
>> + *     requirement.
>> + *   If bit 30 is enabled(checksum with heade mode), HDR_LEN has no
>> need to
...
>> +	DMA_PKT_DROP_EN,
>> +};
> What is the use of this enum if only "DMA_PKT_DROP_DIS" is used in the code?
ok, i will drop the enum and just define it.
>
>> +
>> +enum ldma_endian {
>> +	DMA_ENDIAN_TYPE0 = 0,
...
>> +	DMA_ENDIAN_TYPE3,
>> +};
> And from this enum only "DMA_ENDIAN_TYPE0" is used (indirectly via a define)
same here also.
>
>> +
>> +enum {
...
>> +	PORT5_PCM,
>> +};
> And this enum is not used at all!
...
>> +	const char	*name;
>> +	u32 type;
> Why are the element names not aligned, as in other structs?
thanks, i will update in the next patch.
>
>> +};
>> +
>> +struct ldma_dev {
...
> typo: exclusive
Thank you very much. i will update it in the next patch.
>
>> +	u32			chan_nrs;
>> +	u32			port_nrs;
>> +	u32                     channels_mask;
> Why do you use space in this line?
Missed. i will fix it.
>
>> +	u32			flags;
>> +	u32			pollcnt;
...
>> +		u32 c		:1;
>> +		u32 own		:1;
> Typically, bitfields are written without spaces.
> And you are really sure that this driver will never be used for the older SoCs (MIPS based)?
> Because bitfields are difficult to handle when it comes to support both endianess variants.
This is driver is intended only for LGM and later versions. i won't 
support older SoCs.
>
>> +	} __packed field;
>> +	u32 addr;
...
>> +}
>> +
>> +static inline struct ldma_dev *to_ldma_dev(struct dma_device
>> *dma_dev)
>> +{
>> +	return container_of(dma_dev, struct ldma_dev, dma_dev);
>> +}
>> +
>> +static inline struct dw2_desc_sw *to_lgm_dma_desc(struct
>> virt_dma_desc *vdesc)
>> +{
...
>> +}
>> +
>> +static void ldma_dev_chan_flow_ctl_cfg(struct ldma_dev *d, bool
>> enable)
>> +{
>> +	unsigned long flags;
>> +	u32 mask, val;
...
>> +}
>> +
>> +static void ldma_dev_desc_fetch_on_demand_cfg(struct ldma_dev *d,
>> bool enable)
>> +{
>> +	unsigned long flags;
>> +	u32 mask, val;
...
>> +}
>> +
>> +static void ldma_dev_df_tout_cfg(struct ldma_dev *d, bool enable, int
>> tcnt)
>> +{
...
>> +	u32 mask, val;
>> +
>> +	if (d->inst->type != DMA_TYPE_RX && d->inst->type !=
>> DMA_TYPE_MCPY)
>> +		return;
>> +
...
>> +}
>> +
>> +static void ldma_dev_vld_fetch_ack_cfg(struct ldma_dev *d, bool
>> enable)
>> +{
>> +	unsigned long flags;
>> +	u32 mask, val;
...
>> +
>> +	ldma_update_bits(d, DMA_CS_MASK, c->nr, DMA_CS);
>> +	ldma_update_bits(d, DMA_CCTRL_CLASS | DMA_CCTRL_CLASSH,
>> class_val,
>> +			 DMA_CCTRL);
>> +}
>> +
...
>> +}
>> +
>> +static void ldma_chan_desc_hw_cfg(struct ldma_chan *c, dma_addr_t
>> desc_base,
>> +				  int desc_num)
>> +{
>> +	struct ldma_dev *d = to_ldma_dev(c->vchan.chan.device);
...
>> +
>> +/*
>> + * Descriptor base address and data pointer must be physical address
>> when
>> + * writen to the register.
>> + * This API will be used by CBM which configure hardware descriptor.
>> + */
>> +static int ldma_chan_desc_cfg(struct ldma_chan *c, dma_addr_t
>> desc_base,
>> +			      int desc_num)
>> +{
>> +	struct ldma_dev *d = to_ldma_dev(c->vchan.chan.device);
...
>> +			    int desc_num)
>> +{
>> +	return ldma_chan_desc_cfg(to_ldma_chan(chan), desc_base,
>> desc_num);
>> +}
...
>> +}
>> +
>> +static void ldma_chan_byte_offset_cfg(struct ldma_chan *c, u32
>> boff_len)
>> +{
>> +	struct ldma_dev *d = to_ldma_dev(c->vchan.chan.device);
...
>> +		val = FIELD_PREP(DMA_C_BOFF_BOF_LEN, boff_len) |
>> DMA_C_BOFF_EN;
>> +	else
>> +		val = 0;
>> +
...
>> +}
>> +
>> +static void ldma_chan_data_endian_cfg(struct ldma_chan *c, bool
>> enable,
>> +				      u32 endian_type)
>> +{
>> +	struct ldma_dev *d = to_ldma_dev(c->vchan.chan.device);
...
>> +
>> +	if (enable)
>> +		val = DMA_C_END_DE_EN | FIELD_PREP(DMA_C_END_DATAENDI,
>> endian_type);
>> +	else
>> +		val = 0;
>> +
...
>> +}
>> +
>> +static void ldma_chan_desc_endian_cfg(struct ldma_chan *c, bool
>> enable,
>> +				      u32 endian_type)
>> +{
>> +	struct ldma_dev *d = to_ldma_dev(c->vchan.chan.device);
...
>> +
>> +	if (enable)
>> +		val = DMA_C_END_DES_EN | FIELD_PREP(DMA_C_END_DESENDI,
>> endian_type);
>> +	else
>> +		val = 0;
>> +
...
>> +}
>> +
>> +static void ldma_chan_hdr_mode_cfg(struct ldma_chan *c, u32 hdr_len,
>> bool csum)
>> +{
>> +	struct ldma_dev *d = to_ldma_dev(c->vchan.chan.device);
>> +	u32 mask, val;
...
>> +			ldma_chan_desc_hw_cfg(c, c->ds->desc_phys, c->ds-
>>> desc_cnt);
>> +			ldma_chan_irq_en(c);
>> +		}
>> +		spin_unlock_irqrestore(&c->vchan.lock, flags);
...
>> +		return;
>> +
>> +	writel(readl(d->base + DMA_CIE) & ~DMA_CI_ALL, d->base +
>> DMA_CIE);
...
>> +
>> +static void
>> +update_burst_len_v22(struct ldma_chan *c, struct ldma_port *p, u32
>> burst)
>> +{
>> +	if (ldma_chan_tx(c))
>> +		p->txbl = ilog2(burst);
...
>> +
>> +static void
>> +update_burst_len_v3X(struct ldma_chan *c, struct ldma_port *p, u32
>> burst)
>> +{
>> +	if (ldma_chan_tx(c))
>> +		p->txbl = burst;
...
>> +
>> +static int
>> +update_client_configs(struct of_dma *ofdma, struct of_phandle_args
>> *spec)
>> +{
...
>> +}
>> +
>> +static int ldma_init_v22(struct ldma_dev *d, struct platform_device
>> *pdev)
>> +{
>> +	int ret;
>> +
>> +	ret = device_property_read_u32(d->dev, "dma-channels", &d-
>>> chan_nrs);
>> +	if (ret < 0) {
...
>> +	.device_pause = dma_pause_chan,
>> +};
> It is not easy to see, as the ordering is different, but all the functions
> for hdma_ops are the same as for dma0_ops. And these structs are only used
> as template, assigning the function pointers one by one to the dma_device struct.
> So why not assign the functions directly in the probe function,
> with a check for "dma0" on the slave functions?
> Anyway, if you keep this, you should change them to "const".
...
>> +	{}
>> +};
> If you accept the suggestion I did on the binding, the ".data" should
> probably point directly to the ".ops", or just a flag for the type.
> Which also makes the "name" in the structs obsolete, as they are not
...
>> +	}
>> +
>> +	ret = device_property_read_u32(dev, "dma-channel-mask", &d-
>>> channels_mask);
>> +	if (ret < 0)
...
>> +	d->ports = devm_kcalloc(dev, d->port_nrs, sizeof(*p),
>> GFP_KERNEL);
>> +	if (!d->ports)
>> +		return -ENOMEM;
>> +
>> +	/* Channels Initializations */
>> +	d->chans = devm_kcalloc(d->dev, d->chan_nrs, sizeof(*c),
>> GFP_KERNEL);
>> +	if (!d->chans)
>> +		return -ENOMEM;
>> +
...
>> +	dma_dev->device_terminate_all = d->inst->ops-
>>> device_terminate_all;
>> +	dma_dev->device_issue_pending = d->inst->ops-
...
>> +	dma_dev->device_prep_slave_sg = d->inst->ops-
...
>> +	if (ret) {
>> +		dev_err(dev, "Failed to register slave DMA engine
>> device\n");
>> +		return ret;
>> +	}
>> +
>> +	ret = of_dma_controller_register(pdev->dev.of_node, ldma_xlate,
>> d);
>> +	if (ret) {
...
>> happens
>> + * before its DMA clients of some are platform specific and also to
>> provide
>> + * registered DMA channels and DMA capabilities to clients before
>> their
>> + * initialization.
>> + */
...
> We already have 2020
thanks, i will update to 2020.
>
>> + */
>> +#ifndef LGM_DMA_H
>> +#define LGM_DMA_H
...
> The term "CBM" is not known to the community.
> If this is needed for the CBM driver, I would suggest adding it together with that driver.
"CBM" refers central buffer manager. Next we are going to upstream "CBM" 
as a part of datapath drivers.
>
>> + */
>> +int intel_dma_chan_desc_cfg(struct dma_chan *chan, dma_addr_t
>> desc_base,
